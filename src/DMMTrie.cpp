...
void IndexNode::AddChild(int index, Node *child, uint64_t version,
  ...
  if (index >= 0 && index < DMM_NODE_FANOUT) {
    ...
    throw runtime_error("AddChild out of range. Index: "+index);
}
...
//DeltaPage::DeltaPage(char * page_data, const string& pid): Page(page_data, {0,0,false,pid}){}
  ...
  if (deltapage != nullptr) {
    ...
    //从这开始注释的
     if (deltapage->GetDeltaPageUpdateCount() >= Td_) {
       ...
       worker_->AddDeltaPageVersion(pagekey.pid, version);//更新pid的版本
     }
...
bool DMMTrie::Put(uint64_t tid, uint64_t version, const string &key,
  const string& value) {
  master_->Put(tid, version, key, value);
  return true;
...
string DMMTrie::Get(uint64_t tid, uint64_t version, const string& key) {
  return master_->Get(tid, version, key);
...
DMMTrieProof DMMTrie::GetProof(uint64_t tid, uint64_t version,
  const string& key) {
  return master_->GetProof(tid, version, key);
