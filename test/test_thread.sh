...
COMMITS=16           # 提交次数
BASELINE_TIME=63.32   # 基准线(秒)

# === 线程范围 ===
MIN_THREADS=49        # 最小线程数 (对应MAX_REGION_NUM)
MAX_THREADS=64       # 最大线程数 (对应MAX_REGION_NUM)
REPEAT_PER_THREAD=1  # 每线程重复次数
...
# === 输出文件 ===
RESULTS_FILE="$SCRIPT_DIR/thread_performance_results.csv"
...
# 主测试循环
echo "开始性能测试 (线程范围: $MIN_THREADS-$MAX_THREADS)..."
...
for thread_num in $(seq $MIN_THREADS $MAX_THREADS); do
    sum_time=0
    success_count=0
